...
## Sequence
   ...
   ```sequence
   dongle --> echo: MCTT (JKEY_ISSUEID_MOTION)
   ```
...
#define JKEY_ISSUEID_MOTION                      0x0001000C // motion
   ```
### Command - JVAL_METHODID_PUT & JKEY_ISSUEID_MOTION
   ```
...
#### Sample
   ...
   ```
### Report - JVAL_METHODID_EVENT & JKEY_ISSUEID_MOTION
   ```
typedef struct
{
...
zwifd_alrm_motion_report(ZWARE_REPORTER(), alarm_info);

zwifd_alrm_motion_on_report_bin(ZWARE_REPORTER());
zwifd_alrm_motion_off_report_bin(ZWARE_REPORTER());
   ```

# Log
## MCTT
   ```
20210414 133902 [0/2-ZWAVES2] - <-- 9C65F9361C00/D808ACB4/0x0002/0 - zevent=0x0002.0,motion,off
   ```

## TOPIC
   ```
[3613/3614] bee_topic_issue_cb:92 - (topic: 0/2/9C65F9361C00/D808ACB4/2/0/0001000C, value: {"class":"Motion Sensor","val":"idle"})
   ```
