...
static void tracker_others_cb(void *userdata, IssueItem_t *c_issueitem)
{
	...
	for (cur = clist_head(head); cur != NULL; cur = clist_item_next(cur))
	{
		if ((SAFE_STRCMP(c_uuid, JVAL_C_UUID_BROADCAST) == 0)  || (SAFE_STRCMP(cur->c_uuid, JVAL_C_UUID_BROADCAST) == 0) || (SAFE_STRCMP(cur->c_uuid, c_uuid) == 0))
		{
			if (cur->others_cb)
			{
				cur->others_cb(userdata, c_issueitem);
			}
		}
	}
}
...
void tracker_others_simple(void *userdata, IssueItem_t *c_issueitem, uint8_t dbg)
{
	if (dbg==0)
	{
		tracker_others_cb(userdata, c_issueitem);
	}
}
...
static void tracker_order_func_cb(void *userdata, IssueItem_t *c_issueitem, ORDER_FN_ID fn_id)
{
	...
	for (cur = clist_head(head); cur != NULL; cur = clist_item_next(cur))
	{
		...
		if ((SAFE_STRCMP(c_uuid, JVAL_C_UUID_BROADCAST) == 0)  || (SAFE_STRCMP(cur->c_uuid, JVAL_C_UUID_BROADCAST) == 0) || (SAFE_STRCMP(cur->c_uuid, c_uuid) == 0))
		{
			if (cur->order_item_cb[fn_id])
			{
				cur->order_item_cb[fn_id](userdata, c_issueitem);
			}
		}
	}
}
...
static void tracker_order_cb(void *userdata, IssueItem_t *c_issueitem)
{
	...
	for (cur = clist_head(head); cur != NULL; cur = clist_item_next(cur))
	{
		...
		if ((SAFE_STRCMP(c_uuid, JVAL_C_UUID_BROADCAST) == 0) || (SAFE_STRCMP(cur->c_uuid, JVAL_C_UUID_BROADCAST) == 0) || (SAFE_STRCMP(cur->c_uuid, c_uuid) == 0))
		{
			if (cur->order_cb)
			{
				cur->order_cb(userdata, c_issueitem);
			}
		}
	}
}
...
void tracker_order_simple(void *userdata, IssueItem_t *c_issueitem, uint8_t dbg)
{
	...
	switch (JKEY_ISSUEID_GATEWAY_MASK & issueid)
	{
		...
		case JKEY_ISSUEID_GW:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_GW);
			}
			break;
		// 0x00000002
		case JKEY_ISSUEID_ADDING:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_ADDING);
			}
			break;
		// 0x00000003
		case JKEY_ISSUEID_REMOVING:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_REMOVING);
			}
			break;
		// 0x00000004
		case JKEY_ISSUEID_ABORTING:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_ABORTING);
			}
			break;
		// 0x00000005
		case JKEY_ISSUEID_LEARNING:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_LEARNING);
			}
			break;
		// 0x00000007
		case JKEY_ISSUEID_NW:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_NW);
			}
			break;
		// 0x00000008
		case JKEY_ISSUEID_RESETING:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_RESETING);
			}
			break;
		// 0x0000000A
		case JKEY_ISSUEID_REPLACING:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_REPLACING);
			}
			break;
		// 0x0000000B
		case JKEY_ISSUEID_RMFAILED:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_RMFAILED);
			}
			break;
		// 0x00000010
		case JKEY_ISSUEID_NODE:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_NODE);
			}
			break;
		// 0x00000011
		case JKEY_ISSUEID_NOP:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_NOP);
			}
			break;
		// 0x00092001
		case JKEY_ISSUEID_CC_BASIC:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_BASIC);
			}
			break;
		// 0x00092501
		case JKEY_ISSUEID_CC_SWITCH:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_SWITCH);
			}
			break;
		// 0x00092601
		case JKEY_ISSUEID_CC_DIMMER:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_LEVEL);
			}
			break;
		...
		case JKEY_ISSUEID_CC_INFRARED:
			...
			if (dbg==0)
			{
				tracker_order_func_cb(userdata, c_issueitem, ORDER_FN_ID_INFRARED);
			}
			break;
	...
	if (dbg==0)
	{
		tracker_order_cb(userdata, c_issueitem);
	}
}
...
static void tracker_watch_func_cb(void *userdata, IssueItem_t *c_issueitem, NOTIFY_FN_ID fn_id)
{
	...
	for (cur = clist_head(head); cur != NULL; cur = clist_item_next(cur))
	{
		if ((SAFE_STRCMP(c_uuid, JVAL_C_UUID_BROADCAST) == 0)  || (SAFE_STRCMP(cur->c_uuid, JVAL_C_UUID_BROADCAST) == 0) || (SAFE_STRCMP(cur->c_uuid, c_uuid) == 0))
		{
			if (cur->watch_item_cb[fn_id])
			{
				cur->watch_item_cb[fn_id](userdata, c_issueitem);
			}
		}
	}
}
...
static void tracker_watch_cb(void *userdata, IssueItem_t *c_issueitem)
{
	...
	for (cur = clist_head(head); cur != NULL; cur = clist_item_next(cur))
	{
		if ((SAFE_STRCMP(c_uuid, JVAL_C_UUID_BROADCAST) == 0)  || (SAFE_STRCMP(cur->c_uuid, JVAL_C_UUID_BROADCAST) == 0) || (SAFE_STRCMP(cur->c_uuid, c_uuid) == 0))
		{
			if (cur->watch_cb)
			{
				cur->watch_cb(userdata, c_issueitem);
			}
		}
	}
}
...
void tracker_watch_simple(void *userdata, IssueItem_t *c_issueitem, uint8_t dbg)
{
	...
	switch (issueid)
	{
		...
		case JKEY_ISSUEID_GW:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_GW);
			}
			break;
		// 0x00000001
		case JKEY_ISSUEID_READY:
		{
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_READY);
			}
		}
		break;
		// 0x00000002
		case JKEY_ISSUEID_ADDING:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_ADDING);
			}
			break;
		// 0x00000003
		case JKEY_ISSUEID_REMOVING:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_REMOVING);
			}
			break;
		// 0x00000004
		case JKEY_ISSUEID_ABORTING:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_ABORTING);
			}
			break;
		// 0x00000005
		case JKEY_ISSUEID_LEARNING:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_LEARNING);
			}
			break;
		// 0x00000007
		case JKEY_ISSUEID_NW:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_NW);
			}
			break;
		// 0x00000008
		case JKEY_ISSUEID_RESETING:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_RESETING);
			}
			break;
		// 0x00000009
		case JKEY_ISSUEID_OP:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_OP);
			}
			break;
		// 0x0000000A
		case JKEY_ISSUEID_REPLACING:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_REPLACING);
			}
			break;
		// 0x0000000B
		case JKEY_ISSUEID_RMFAILED:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_RMFAILED);
			}
			break;
		// 0x00000010
		case JKEY_ISSUEID_NODE:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_NODE);
			}
			break;
		// 0x00000011
		case JKEY_ISSUEID_NOP:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_NOP);
			}
			break;
		// 0x00010002
		case JKEY_ISSUEID_SMOKE:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_SMOKE);
			}
			break;
		// 0x00010006
		case JKEY_ISSUEID_WATER:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_WATER);
			}
			break;
		// 0x00010008
		case JKEY_ISSUEID_TAMPER:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_TAMPER);
			}
			break;
		// 0x0001000A
		case JKEY_ISSUEID_DR_WIN:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_DR_WIN);
			}
			break;
		// 0x0001000C
		case JKEY_ISSUEID_MOTION:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_MOTION);
			}
			break;
		// 0x00030001
		...
		if (dbg==0)
		{
			tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_TEMPATURE);
		}
		break;
		// 0x00030003
		case JKEY_ISSUEID_LUM:
		{
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_LUMINANCE);
			}
		}
		break;
		// 0x00030005
		case JKEY_ISSUEID_HUMD:
		{
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_HUMIDITY);
			}
		}
		break;
		// 0x00092001
		case JKEY_ISSUEID_CC_BASIC:
		{
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_BASIC);
			}
		}
		break;
		// 0x00092002
		case JKEY_ISSUEID_CC_BASIC_SET:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_BASIC_SET);
			}
			break;
		// 0x00092501
		case JKEY_ISSUEID_CC_SWITCH:
		...
		case JKEY_ISSUEID_CC_SWITCH_PLUG:
		{
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_SWITCH);
			}
		}
		break;
		// 0x00092502
		...
		case JKEY_ISSUEID_CC_DIMMER:
		{
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_DIMMER);
			}
		}
		break;
		// 0x00092701
		...
		if (dbg==0)
		{
			tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_METER);
		}
		break;
		// 0x00093202
		...
		case JKEY_ISSUEID_CC_CENTRAL_SCENE_NOTIFICATION:
		{
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_CENTRAL_SCENE_NOTIFICATION);
			}
		}
		break;
		// 0x00095b02
		...
		case JKEY_ISSUEID_CC_CONFIGURATION:
		{
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_CONFIGURATION);
			}
		}
		break;
		// 0x00097501
		...
		case JKEY_ISSUEID_CC_BATTERY:
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_BATTERY);
			}
			break;
		// 0x00098401
		case JKEY_ISSUEID_CC_WAKE_UP_INTERVAL:
		{
			uint32_t imin = byte2big_endian(4, (uint8_t *)valp+idx);
			idx+=4;
			uint32_t imax = byte2big_endian(4, (uint8_t *)valp+idx);
			idx+=4;
			uint32_t idef = byte2big_endian(4, (uint8_t *)valp+idx);
			idx+=4;
			uint32_t interval = byte2big_endian(4, (uint8_t *)valp+idx);
			idx+=4;
			uint32_t cur = byte2big_endian(4, (uint8_t *)valp+idx);
			idx+=4;
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_WAKE_UP_INTERVAL);
			}
		}
		break;
		// 0x00098402
		case JKEY_ISSUEID_CC_WAKE_UP_NOTIFICATION:
		{
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_WAKE_UP_NOTIFICATION);
			}
		}
		break;
		// 0x00098501
		...
		case JKEY_ISSUEID_CC_INFRARED:
		{
			...
			if (dbg==0)
			{
				tracker_watch_func_cb(userdata, c_issueitem, NOTIFY_FN_ID_INFRARED);
			}
		}
		break;
		default:
	...
	if (dbg==0)
	{
		tracker_watch_cb(userdata, c_issueitem);
	}
}
