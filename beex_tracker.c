...
{
	SAFE_SPRINTF(my_macid, "%s", macid);
}

void tracker_notify_set_uuid(Notify_t *notify, char *c_uuid)
{
	SAFE_SPRINTF(notify->c_uuid, "%s", c_uuid);
}
...
void tracker_order_simple(void *userdata, IssueItem_t *c_issueitem, uint8_t dbg)
{
	...
	SAFE_SPRINTF(node_str, "%s.%d", c_nodeid_to, epid_to);
	SAFE_SPRINTF(response, "0x%08X,%u", issueid, val);
	SAFE_SPRINTF(tag, "%s", JKEY_REPORT_COMMAND );
	...
	switch (JKEY_ISSUEID_GATEWAY_MASK & issueid)
	{
		...
		case JKEY_ISSUEID_GW:
			SAFE_SPRINTF(response, "%s,", JKEY_REPORT_GATEWAY);
		...
		case JKEY_ISSUEID_ADDING:
			SAFE_SPRINTF(response, "%s,", JKEY_REPORT_OP_ADDING);
			BUFF_DUMP_BUFF(valp, data_len, ",", "0x", buff, LEN_OF_BUF256);
			SAFE_STRCAT(response, buff);
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_COMMAND );
		...
		case JKEY_ISSUEID_REMOVING:
			SAFE_SPRINTF(response, "%s,", JKEY_REPORT_OP_REMOVING);
			BUFF_DUMP_BUFF(valp, data_len, ",", "0x", buff, LEN_OF_BUF256);
			SAFE_STRCAT(response, buff);
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_COMMAND );
		...
		case JKEY_ISSUEID_ABORTING:
			SAFE_SPRINTF(response, "%s,", JKEY_REPORT_OP_ABORTING);
			BUFF_DUMP_BUFF(valp, data_len, ",", "0x", buff, LEN_OF_BUF256);
			SAFE_STRCAT(response, buff);
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_COMMAND );
		...
		case JKEY_ISSUEID_LEARNING:
			SAFE_SPRINTF(response, "%s,", JKEY_REPORT_OP_LEARNING);
			BUFF_DUMP_BUFF(valp, data_len, ",", "0x", buff, LEN_OF_BUF256);
			SAFE_STRCAT(response, buff);
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_COMMAND );
		...
		case JKEY_ISSUEID_RESETING:
			SAFE_SPRINTF(response, "%s,", JKEY_REPORT_OP_RESETING);
			BUFF_DUMP_BUFF(valp, data_len, ",", "0x", buff, LEN_OF_BUF256);
			SAFE_STRCAT(response, buff);
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_COMMAND );
		...
		case JKEY_ISSUEID_REPLACING:
			SAFE_SPRINTF(response, "%s,", JKEY_REPORT_OP_REPLACING_FAILED);
			BUFF_DUMP_BUFF(valp, data_len, ",", "0x", buff, LEN_OF_BUF256);
			SAFE_STRCAT(response, buff);
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_COMMAND );
		...
		case JKEY_ISSUEID_RMFAILED:
			SAFE_SPRINTF(response, "%s,", JKEY_REPORT_OP_REMOVING_FAILED);
			BUFF_DUMP_BUFF(valp, data_len, ",", "0x", buff, LEN_OF_BUF256);
			SAFE_STRCAT(response, buff);
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_COMMAND );
		...
		case JKEY_ISSUEID_NOP:
			SAFE_SPRINTF(response, "%s,", JKEY_REPORT_NOP);
			BUFF_DUMP_BUFF(valp, data_len, ",", "0x", buff, LEN_OF_BUF256);
			SAFE_STRCAT(response, buff);
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_COMMAND );
		...
		case JKEY_ISSUEID_CC_BASIC:
			SAFE_SPRINTF(response, "%s,", JKEY_REPORT_BASIC);
			BUFF_DUMP_BUFF(valp, data_len, ",", "0x", buff, LEN_OF_BUF256);
			SAFE_STRCAT(response, buff);
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_COMMAND );
		...
		case JKEY_ISSUEID_CC_SWITCH:
			SAFE_SPRINTF(response, "%s,", JKEY_REPORT_SWITCH);
			BUFF_DUMP_BUFF(valp, data_len, ",", "0x", buff, LEN_OF_BUF256);
			SAFE_STRCAT(response, buff);
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_COMMAND );
		...
		case JKEY_ISSUEID_CC_DIMMER:
			SAFE_SPRINTF(response, "%s,", JKEY_REPORT_DIMMER);
			BUFF_DUMP_BUFF(valp, data_len, ",", "0x", buff, LEN_OF_BUF256);
			SAFE_STRCAT(response, buff);
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_COMMAND );
	...
	if (tracker_order==1)
	{
		...
		SAFE_SPRINTF(id_frm, "%s/%s/%s/%d", c_macid_frm, c_uuid_frm, c_nodeid_frm, epid_frm);
		char id_to[LEN_OF_BUF256] = "";
		SAFE_SPRINTF(id_to, "%s/%s/%s/%d", c_macid_to, c_uuid_to, c_nodeid_to, epid_to);
...
void tracker_watch_simple(void *userdata, IssueItem_t *c_issueitem, uint8_t dbg)
{
	...
	char buff[LEN_OF_BUF256] = "";
	char node_str[LEN_OF_UUID+8] = "";
	SAFE_SPRINTF(node_str, "%s.%d", c_nodeid_frm, epid_frm);
	SAFE_SPRINTF(response, "0x%08X,", issueid);
	BUFF_DUMP_BUFF(valp, data_len, ",", "0x", buff, LEN_OF_BUF256);
	SAFE_STRCAT(response, buff);
	switch (issueid)
	{
				...
				case ZWAVE_EVT_GW_CFG:
					{
						...
						SAFE_SPRINTF(response, "%s,%u,%d", JKEY_REPORT_CFG, param_num, value);
					}
					break;
				// 96
				case ZWAVE_EVT_GW_CMD:
					{
						SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_COMMAND, valp+idx);
					}
					break;
				// 97
				case ZWAVE_EVT_GW_DEBUG:
					{
						SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_DEBUG, valp+idx);
					}
					break;
				// 98
				case ZWAVE_EVT_GW_WATCH_OFF:
					{
						SAFE_SPRINTF(response, "%s,%u", JKEY_REPORT_WATCH_OFF, valp[idx++]);
					}
					break;
				// 99
				case ZWAVE_EVT_GW_ECHO:
					{
						SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_ECHO, valp+idx);
					}
					break;
				default:
					{
						SAFE_SPRINTF(response, "0x%08x,%u", JKEY_ISSUEID_GW, val);
					}
					break;
			}
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_ZWINFO );
		...
		case JKEY_ISSUEID_READY:
			{
				...
				idx+=2;
				SAFE_SPRINTF(response, "%s,%s,0x%02X", JKEY_REPORT_READY, translate_nw_op(last_op), nodeid_ready);
				SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
		...
		case JKEY_ISSUEID_ADDING:
			...
			switch (val)
			{
				case ZWAVE_EVT_ADDING_START:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_ADDING, JVAL_REPORT_START);
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_ADDING_ADDED:
					SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_OP_ADDED );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_ADDING_DONE:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_ADDING, JVAL_REPORT_DONE);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				case ZWAVE_EVT_ADDING_FAIL:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_ADDING, JVAL_COMM_FAIL);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				case ZWAVE_EVT_ADDING_CSA:
					SAFE_SPRINTF(response, "%s,%s,%s", JKEY_REPORT_OP_ADDING, JVAL_SECURITY_CSA, valp+idx);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				case ZWAVE_EVT_ADDING_SSA_SET:
					SAFE_SPRINTF(response, "%s,%s,%u", JKEY_REPORT_OP_ADDING, JVAL_SECURITY_SET, valp[idx++]);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				case ZWAVE_EVT_ADDING_SSA:
					SAFE_SPRINTF(response, "%s,%s,%s", JKEY_REPORT_OP_ADDING, JVAL_SECURITY_SSA, valp+idx);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				case ZWAVE_EVT_ADDING_SEC:
					{
						...
						SAFE_SPRINTF(response, "%s,%s,%d,%d,%s", JKEY_REPORT_OP_ADDING, JKEY_REPORT_SEC, highest_lvl, curr_lvl, (curr_lvl<highest_lvl)? JVAL_SECURITY_LOWER: JVAL_SECURITY_NORMAL );
						SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					}
					break;
				default:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_ADDING, translate_op_add_node(val) );
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
			}
		...
		case JKEY_ISSUEID_REMOVING:
			...
			switch (val)
			{
				case ZWAVE_EVT_REMOVING_START:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_REMOVING, JVAL_REPORT_START);
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_REMOVING_REMOVED:
					SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_OP_REMOVED );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_REMOVING_DONE:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_REMOVING, JVAL_REPORT_DONE);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				case ZWAVE_EVT_REMOVING_FAIL:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_REMOVING, JVAL_COMM_FAIL);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				default:
					SAFE_SPRINTF(response, "%s,%d", JKEY_REPORT_OP_REMOVING, val );
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
			}
		...
		case JKEY_ISSUEID_ABORTING:
			...
			switch (val)
			{
				case ZWAVE_EVT_ABORTING_ABORTED:
					SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_OP_ABORTED );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_ABORTING_DONE:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_ABORTING, JVAL_REPORT_DONE);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				case ZWAVE_EVT_ABORTING_FAIL:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_ABORTING, JVAL_COMM_FAIL);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				default:
					SAFE_SPRINTF(response, "%s,%d", JKEY_REPORT_OP_ABORTING, val );
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
			}
		...
		case JKEY_ISSUEID_LEARNING:
			...
			switch (val)
			{
				case ZWAVE_EVT_LEARNING_START:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_LEARNING, JVAL_REPORT_START);
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_LEARNING_LEARNED:
					SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_OP_LEARNED );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_LEARNING_DONE:
					SAFE_SPRINTF(response, "%s,%s,%d", JKEY_REPORT_OP_LEARNING, JVAL_REPORT_DONE, valp[idx++]);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				case ZWAVE_EVT_LEARNING_FAIL:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_LEARNING, JVAL_COMM_FAIL);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				case ZWAVE_EVT_ADDING_SSA_SET:
					SAFE_SPRINTF(response, "%s,%s,%u", JKEY_REPORT_OP_ADDING, JVAL_SECURITY_SET, valp[idx++]);
					...
					break;
				case ZWAVE_EVT_ADDING_SSA:
					SAFE_SPRINTF(response, "%s,%s,%s", JKEY_REPORT_OP_LEARNING, JVAL_SECURITY_SSA, valp+idx);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				default:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_LEARNING, translate_op_initiate(val) );
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
			}
		...
		case JKEY_ISSUEID_NW:
			...
			switch (val)
			{
				case ZWAVE_EVT_NW_NOROUTE:
					SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_NW_NOROUTE );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_NW_BUSY:
					SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_NW_BUSY );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_NW_NO_ACK:
					SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_NW_NO_ACK );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_NW_SYS_ERR:
					SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_NW_SYS_ERR );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_NW_BOOTUP:
					SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_NW_BOOTUP );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_NW_SERIAL:
					{
						...
						SAFE_SPRINTF(response, "%s,%u,0x%02x,%u,%u,0x%02x", JKEY_REPORT_NW_SERIAL, moduleVer, capability, chipType, chipVer, cap);
						SAFE_SPRINTF(tag, "%s", JKEY_REPORT_ZWINFO );
					}
					break;
				case ZWAVE_EVT_NW_HOMEID:
					{
						...
						SAFE_SPRINTF(response, "%s,%s,0x%02x,%u-%s,%u-%s,%u-%s,0x%02x", JKEY_REPORT_NW_HOMEID, homeid, ctl_id, ctl_role, ctl_role_name, ctl_cap, translate_ctl_capabilities(ctl_cap), ctl_zw_role, translate_ctl_zw_role(ctl_zw_role), suc);
						SAFE_SPRINTF(tag, "%s", JKEY_REPORT_ZWINFO );
					}
					break;
				case ZWAVE_EVT_NW_DEBUG:
					{
						SAFE_SPRINTF(response, "%s,%d", JKEY_REPORT_DEBUG, valp[idx++]);
						SAFE_SPRINTF(tag, "%s", JKEY_REPORT_ZWINFO );
					}
					break;
				case ZWAVE_EVT_NW_WATCH_OFF:
					{
						SAFE_SPRINTF(response, "%s,%d", JKEY_REPORT_WATCH_OFF, valp[idx++]);
						SAFE_SPRINTF(tag, "%s", JKEY_REPORT_ZWINFO );
					}
					break;
				case ZWAVE_EVT_NW_ECHO:
					{
						SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_ECHO, valp+1);
						SAFE_SPRINTF(tag, "%s", JKEY_REPORT_ZWINFO );
					}
					break;
				default:
					SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_NW_XXX );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
			}
		...
		case JKEY_ISSUEID_RESETING:
			...
			switch (val)
			{
				...
				case ZWAVE_EVT_RESETING_REMOVED:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_RESET, valp+idx);
					break;
				case ZWAVE_EVT_RESETING_FAIL:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_RESET, JVAL_COMM_FAIL);
					break;
				default:
					SAFE_SPRINTF(response, JKEY_REPORT_OP_RESET);
					break;
			}
			SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
		...
		case JKEY_ISSUEID_OP:
			if (val==0)
			{
				SAFE_SPRINTF(response, JKEY_REPORT_OP_NORMAL);
			}
			else
			{
				SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP, translate_nw_op(val) );
			}
			SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
		...
		case JKEY_ISSUEID_REPLACING:
			...
			switch (val)
			{
				case ZWAVE_EVT_REPLACING_START:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_REPLACING_FAILED, JVAL_REPORT_START);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				case ZWAVE_EVT_REPLACING_DONE:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_REPLACING_FAILED, JVAL_REPORT_DONE);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				case ZWAVE_EVT_REPLACING_REPLACED:
					SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_OP_REPLACED_FAILED );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_REPLACING_FAIL:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_REPLACING_FAILED, JVAL_COMM_FAIL);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				default:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_REPLACING_FAILED, translate_op_rp_node(val) );
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
			}
		...
		case JKEY_ISSUEID_RMFAILED:
			...
			switch (val)
			{
				case ZWAVE_EVT_RMFAILED_REMOVED:
					SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_OP_REMOVED_FAILED );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_RMFAILED_DONE:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_REMOVING_FAILED, JVAL_REPORT_DONE);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				case ZWAVE_EVT_RMFAILED_FAIL:
					SAFE_SPRINTF(response, "%s,%s", JKEY_REPORT_OP_REMOVING_FAILED, JVAL_COMM_FAIL);
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
				default:
					SAFE_SPRINTF(response, "%s,%d", JKEY_REPORT_OP_REMOVING_FAILED, val );
					SAFE_SPRINTF(tag, "%s", JKEY_COMM_STATE );
					break;
			}
		...
		case JKEY_ISSUEID_NODE:
			...
			switch (val)
			{
				case ZWAVE_EVT_NODE_ALIVE:
					SAFE_SPRINTF(response, "%s,%s", node_str, JVAL_NODE_ALIVE );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_NODE_SLEEPING:
					SAFE_SPRINTF(response, "%s,%s", node_str, JVAL_NODE_SLEEPING );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
				case ZWAVE_EVT_NODE_INFO:
					{
						...
						SAFE_SPRINTF(response, "%s,%s,%d-%s,0x%04x,0x%04x,0x%04x,%u.%02u,%u.%02u,%u,%u,%u,%u,%u,%d,%u,0x%02x-%s", node_str, JKEY_REPORT_NODE_INFO,
				...
				case ZWAVE_EVT_NODE_TYPE:
					{
						...
						SAFE_SPRINTF(response, "%s,%u,0x%02x-%s,0x%02x-%s,0x%02x,0x%02x", node_str, zwplus_ver, node_type, translate_node_type(node_type), role_type, translate_role_type(node_type), instr_icon, usr_icon);
						SAFE_SPRINTF(tag, "%s", JKEY_REPORT_ZWINFO );
					}
					break;
				case ZWAVE_EVT_NODE_GENERIC:
					{
						...
						SAFE_SPRINTF(response, "%s,0x%02x-%s,0x%02x-%s", node_str, generic, translate_generic(generic), specific, translate_specific(generic, specific));
						SAFE_SPRINTF(tag, "%s", JKEY_REPORT_ZWINFO );
					}
					break;
				case ZWAVE_EVT_NODE_VENDOR:
					{
						...
						SAFE_SPRINTF(response, "%s,%s,0x%04x,0x%04x,0x%04x", node_str, JKEY_REPORT_NODE_VENDOR, vid, ptype, pid);
						SAFE_SPRINTF(tag, "%s", JKEY_REPORT_ZWINFO );
					}
					break;
				default:
					SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_NODE_XXX );
					SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
					break;
			}
		...
		// 0x00000011
		case JKEY_ISSUEID_NOP:
			SAFE_SPRINTF(response, "%s,%s,", node_str, JKEY_REPORT_NOP );
			SAFE_STRCAT(response, buff);
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_TAMPER:
			...
			{
				case ZWAVE_EVT_INACTIVE_CLEAR:
					SAFE_SPRINTF(response, "%s,%s,%s", node_str, JKEY_REPORT_TAMPER, JVAL_COMM_CLOSED );
					break;
				case ZWAVE_EVT_TMPR_COVER:
				default:
					SAFE_SPRINTF(response, "%s,%s,%s", node_str, JKEY_REPORT_TAMPER, JVAL_COMM_OPEN );
					break;
			}
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_DR_WIN:
			switch (val)
			{
				case ZWAVE_EVT_WINDOW_DOOR_OPEN:
					SAFE_SPRINTF(response, "%s,%s,%s", node_str, JKEY_REPORT_DOOR, JVAL_COMM_OPEN );
					break;
				case ZWAVE_EVT_WINDOW_DOOR_CLOSED:
					SAFE_SPRINTF(response, "%s,%s,%s", node_str, JKEY_REPORT_DOOR, JVAL_COMM_CLOSED);
					break;
			}
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_MOTION:
			...
			{
				case ZWAVE_EVT_INACTIVE_CLEAR:
					SAFE_SPRINTF(response, "%s,%s,%s", node_str, JKEY_REPORT_MOTION, JVAL_COMM_OFF);
					break;
				case ZWAVE_EVT_MOTION_DET_L:
					SAFE_SPRINTF(response, "%s,%s,%s,%s", node_str, JKEY_REPORT_MOTION, JVAL_COMM_ON, valp+ZWAVE_IDX_ALRM_EX_EVT_PRM);
					break;
				case ZWAVE_EVT_MOTION_DET:
				default:
					SAFE_SPRINTF(response, "%s,%s,%s", node_str, JKEY_REPORT_MOTION, JVAL_COMM_ON );
					break;
			}
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_TEMPERATURE:
			{
				...
				SAFE_SPRINTF(response, "%s,%s,%u-%s,%.3f,%u-%s", node_str, JKEY_REPORT_TEMPERATURE, type, translate_sensor_type(type), (float)value/(float)precision, unit, translate_sensor_unit(type, unit));
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
			}
		...
		case JKEY_ISSUEID_LUM:
			{
				...
				SAFE_SPRINTF(response, "%s,%s,%u-%s,%.3f,%u-%s", node_str, JKEY_REPORT_LUMINANCE, type, translate_sensor_type(type), (float)value/(float)precision, unit, translate_sensor_unit(type, unit));
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_HUMD:
			{
				...
				SAFE_SPRINTF(response, "%s,%s,%u-%s,%.3f,%u-%s", node_str, JKEY_REPORT_HUMIDITY, type, translate_sensor_type(type), (float)value/(float)precision, unit, translate_sensor_unit(type, unit));
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_CC_BASIC:
			{
				...
				SAFE_SPRINTF(response, "%s,%s,%u-%s,%u-%s,%u-%s", node_str, JKEY_REPORT_BASIC, curr_val, translate_basic_val(curr_val), tgt_val, translate_basic_val(tgt_val), dur, translate_dur(dur));
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_CC_BASIC_SET:
			SAFE_SPRINTF(response, "%s,%s,%u", node_str, JKEY_REPORT_BASIC_SET, val);
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_CC_SWITCH_PLUG:
			{
				...
				SAFE_SPRINTF(response, "%s,%s,%u-%s,%u-%s,%u-%s", node_str, JKEY_REPORT_SWITCH, curr_val, translate_switch_val(val), tgt_val, translate_switch_val(tgt_val), dur, translate_dur(dur));
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_CC_SWITCH_SIREN:
			{
				SAFE_SPRINTF(response, "%s,%s,%d", node_str, JKEY_REPORT_SIREN, val );
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
			}
			break;
		// 0x00092601
		case JKEY_ISSUEID_CC_DIMMER:
			{
				...
				SAFE_SPRINTF(response, "%s,%s,%u-%s,%u-%s,%u-%s", node_str, JKEY_REPORT_DIMMER, curr_val, translate_multilevel_level(curr_val), tgt_val, translate_multilevel_level(tgt_val), dur, translate_dur(dur));
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_CC_SWITCH_ALL:
			SAFE_SPRINTF(response, "%s,%s,0x%02x", node_str, JKEY_REPORT_SWITCH_ALL, val );
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
			break;
		// 0x00093002
		case JKEY_ISSUEID_CC_BSENSOR_SUP:
			{
				...
				int i = 0;
				SAFE_SPRINTF(response, "%s,%s,%u", node_str, JKEY_REPORT_BSENSOR_SUP, types_len );
				for (i=0; i<types_len; i++ )
				{
					char tmpbuf[LEN_OF_BUF256] = "";
					uint8_t type_v = types[i];
					SAFE_SPRINTF(tmpbuf, ",%u-%s", type_v, translate_bsensor_type(type_v));
		...
		case JKEY_ISSUEID_CC_SENSOR_SUP:
			{
				...
				int i = 0;
				SAFE_SPRINTF(response, "%s,%s,%u", node_str, JKEY_REPORT_SENSOR_SUP, types_len );
				for (i=0; i<types_len; i++ )
				{
					char tmpbuf[LEN_OF_BUF256]="";
					uint8_t type_v = types[i];
					SAFE_SPRINTF(tmpbuf, ",%u-%s", type_v, translate_sensor_type(type_v));
					SAFE_STRCAT(response, tmpbuf);
				}
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
			}
		...
		case JKEY_ISSUEID_CC_SENSOR_UNIT_SUP:
			{
				...
				int i = 0;
				SAFE_SPRINTF(response, "%s,%s,%u-%s,%u", node_str, JKEY_REPORT_SENSOR_UNIT_SUP, sensor_type, translate_sensor_type( sensor_type ), sensor_unit_msk);
				for (i=0; i<4; i++)
				{
					if ( sensor_unit_msk & (uint8_t)pow(2, i))
					{
						char tmpbuf[LEN_OF_BUF256]="";
						SAFE_SPRINTF(tmpbuf, ",%u-%s", i, translate_sensor_unit(sensor_type, i));
						SAFE_STRCAT(response, tmpbuf);
					}
				}
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_CC_METER:
			{
				...
				//SAFE_SPRINTF(response, "%s,%s,%u,%u,%u,%d,%.3f,%.3f", node_str, JSTR_REPORT_METER, typex, unit, rate_type, delta_time, (float)value/(float)precision, (float)prv_value/(float)precision);
				SAFE_SPRINTF(response, "%s,%s,%u-%s,%u-%s,%u-%s,%d-delta_time,%.3f,%.3f", node_str, JKEY_REPORT_METER, 
		...
		case JKEY_ISSUEID_CC_METER_SUP:
			{
				...
				SAFE_SPRINTF(response, "%s,%s,%u-%s,%u-%s,%u-%s", node_str, JKEY_REPORT_METER_SUP, 
		...
		case JKEY_ISSUEID_CC_CONFIGURATION:
			{
				...
				//SAFE_SPRINTF(response, "%s,%s,%u,%u,%u,%u,%u,%u,%u", node_str, JSTR_REPORT_CFG, use_default, param_num, size, data[0], data[1], data[2], data[3]);
				SAFE_SPRINTF(response, "%s,%s,%u-use_default,%u-param_num,%d", node_str, JKEY_REPORT_CFG, use_default, param_num, value);
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_CC_PROTECTION:
			{
				...
				SAFE_SPRINTF(response, "%s,%s,%u-%s,%u-%s", node_str, JKEY_REPORT_PROTECTION, local_prot, translate_local_prot(local_prot), rf_prot, translate_rf_prot(rf_prot));
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
			}
			break;
		// 0x00098001
		case JKEY_ISSUEID_CC_BATTERY:
			...
			SAFE_SPRINTF(response, "%s,%s,%u-%s", node_str, JKEY_REPORT_BATTERY, val, translate_battery_level(val) );
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_CC_WAKE_UP_INTERVAL:
			{
				...
				SAFE_SPRINTF(response, "%s,%s,%d,%d,%d,%d,%d,%u", node_str, JKEY_REPORT_WAKEUP_INTERVAL, imin, imax, idef, interval, cur, nodeid);
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_CC_WAKE_UP_NOTIFICATION:
			{
				SAFE_SPRINTF(response, "%s,%s", node_str, JKEY_REPORT_WAKEUP);
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_CC_GROUP:
			{
				...
				SAFE_SPRINTF(response, "%s,%s,%u-group,%u-max_cnt,%u-cnt", node_str, JKEY_REPORT_GROUP, group, max_cnt, cnt);
				if (cnt>0)
				{
					for (i=0; i<cnt; i++ )
					{
						...
						SAFE_SPRINTF(tmpbuf, ",%u.%u", node_id, ep_id);
						SAFE_STRCAT(response, tmpbuf);
					}
				}
				SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
		...
		case JKEY_ISSUEID_CC_GROUP_SUP:
			SAFE_SPRINTF(response, "%s,%s,%d", node_str, JKEY_REPORT_GROUP_SUP, val );
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
			break;
		// 0x00098503
		case JKEY_ISSUEID_CC_GROUP_ACTVIE:
			SAFE_SPRINTF(response, "%s,%s,%d", node_str, JKEY_REPORT_GROUP_ACTIVE, val );
			SAFE_SPRINTF(tag, "%s", JKEY_REPORT_EVT );
			break;
		default:
			SAFE_SPRINTF(tag, "0x%08x", issueid );
			break;
	}
